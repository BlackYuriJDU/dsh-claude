...
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the DSH Web GUI at ${webUrl}. `
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [DSH_WEB_URL]: { description: 'Canonical local URL of the DSH Web GUI serving this session.' },
        },
