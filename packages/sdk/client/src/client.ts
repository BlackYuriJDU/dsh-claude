/**
 * Low-level JSON-RPC client for a DSH SDK runtime subprocess.
...
/**
 * JSON-RPC client for the DSH SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('DSH runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('DSH runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('DSH runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('DSH runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('DSH runtime is not running')
    ...
    try {
      ...
      const timer = setTimeout(() => {
        abandon.abort(new RequestTimeoutError(`${method} timed out after ${timeout}ms waiting for the DSH runtime`))
      }, timeout)
      try {
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('DSH runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('DSH runtime closed'))
  }
