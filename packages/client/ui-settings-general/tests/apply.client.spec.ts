...
describe('ui-settings-general apply', () => {
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    for (const [name, component] of SEATS) {
      ...
      expect(after.slots.entries(name)).toHaveLength(name === 'settings.section' ? 4 : 1)
    }
    await vi.waitFor(() => {
  ...
  it('registers the settings dictionary and frees the seat on teardown', async () => {
    const b = await bench()
    declare(b.slots)
  ...
  it('the nav label thunk follows the active locale without re-registration', async () => {
    ...
    const beforeVersions = SEATS.map(([name]) => b.slots.getVersion(name))
    b.locale.setLocale('en')
    // No ledger churn: freshness rides the thunk (and the renderer's locale
    // subscription), not re-registration.
    SEATS.forEach(([name], i) => {
      expect(b.slots.getVersion(name)).toBe(beforeVersions[i]!)
      expect(b.slots.entries(name)).toHaveLength(name === 'settings.section' ? 4 : 1)
    })
