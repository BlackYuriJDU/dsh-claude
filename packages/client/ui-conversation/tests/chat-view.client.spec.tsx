...
import { createSnapshotStore, EMPTY_CONVERSATION_VIEWS, PendingWait } from '@deepseek-ai/dsh-client-runtime/client'
...
import { CompactionNodeView, ContextMessageNodeView, RetryNodeView, TurnErrorNodeView, TurnMaxTokensNodeView, UnknownNodeView, UserMessageNodeView } from '../src/client/chat/MessageItem.tsx'
...
describe('ChatView', () => {
  ...
  it('shows a Host open refusal with the reason and retries the same path', async () => {
    ...
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: commonEn['retry'] })) })
    await waitFor(() => {
  ...
  it('renders /compact as one stateful disclosure from running through completion', () => {
    ...
    expect(view.getByText('Compaction summary')).toBeTruthy()
  })
  ...
  it('keeps /compact no-history and error settlements on the generic command row', () => {
    ...
    expect(noHistoryView.container.querySelector('[data-compaction-disclosure]')).toBeNull()
