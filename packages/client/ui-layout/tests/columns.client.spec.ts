...
import { CENTER_MIN, clampWidth, computeColumns, DETAILS_DEFAULT, DETAILS_MIN, SIDEBAR_COLLAPSED, SIDEBAR_DEFAULT, SIDEBAR_MIN } from '@deepseek-ai/dsh-client-ui-layout/src/client/columns.ts'
...
describe('computeColumns', () => {
  ...
  it('step 2: details shrinks first, center pinned at min', () => {
    // 352 + 360 + 640 = 1352 > 1320; details concedes to 1320-352-640 = 328.
    const cols = computeColumns(1320, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: CENTER_MIN, details: 328 })
  })
  ...
  it('step 3: details auto-closes when its min still starves center — sidebar holds its preference', () => {
    // 352 + 300 + 640 = 1292 > 1280 → details 0; sidebar untouched: center = 1280-352 = 928.
    const cols = computeColumns(1280, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: 928, details: 0 })
  })

  it('the sidebar never concedes: center absorbs the deficit below CENTER_MIN', () => {
    // 700 < 352+640: sidebar keeps 352, center takes 348 < CENTER_MIN.
    const cols = computeColumns(700, open(SIDEBAR_DEFAULT), closed(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: 348, details: 0 })
  })
