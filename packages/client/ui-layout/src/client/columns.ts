/**
 * Pure two-column solver for the AppFrame.
 ...
 * SIDEBAR_COLLAPSED control rail.
...
// Contract-frozen geometry: the two-column layout's fixed points.
...
/** Compact desktop sidebar width before any user drag. */
...
/**
 * Solve the two column widths for one viewport frame. Pure: no hysteresis —
 * the output is a function of (viewport, preference) only, so recovery on
 * re-widening is automatic. The preference re-clamps here because it crosses
 * the store boundary and callers may still supply a stale range.
 * @param viewport - available frame width in px.
 * @param sidebar - sidebar width preference in px (0 = closed).
 * @returns resolved widths; a closed sidebar keeps its compact rail.
 */
