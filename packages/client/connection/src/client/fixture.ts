...
function buildAlphaLog(): SessionEvent[] {
  ...
  for (let turn = 0; turn < 60; turn++) {
    ...
    const userSeq = push({
      ...
      data: userMessage(text(turn === 59 ? USER_MARKDOWN_LITERAL : `Question ${turn}: fixture history message for paging and render acceptance.`)),
    })
    if (turn === 0) {
      push({
        type: 'session/title',
    ...
    if (withReasoning) blocks.push({ type: 'reasoning', text: `Reasoning ${turn}: a collapsible reasoning block.` })
    blocks.push({ type: 'text', text: turn === 59 ? MARKDOWN_FIXTURE : `Answer ${turn}: fixture-generated history reply body.` })
    if (withTool) {
  ...
  push({
    ...
    surfaceOp: 'append',
    data: { turn: 72, step: 0, message: assistantMessage(text('Entry 1: first. Entry 2: second. Entry 3: this one stops mid-s')) },
  })
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixtureQuestions: Extract<MuxFrame, { type: 'question/requested' }>['questions'] = [
    {
      id: 'harness-profile',
      header: 'Preference',
      question: 'Which Agent/Harness candidate profile do you prefer right now?',
      options: [
        { label: 'Shipping engineer (Recommended)', description: 'Values direct runtime, tool executor, sandbox, trace, and production debugging experience.' },
        { label: 'Research potential', description: 'Values Agent understanding, training/eval thinking, and long-term growth.' },
        { label: 'Balanced', description: 'Requires both shipping ability and Agent literacy, possibly with a higher bar.' },
      ],
    },
    {
      id: 'work-mode',
      header: 'Approach',
      question: 'Which working style should the candidate showcase first?',
      options: [
        { label: 'Small prototype first (Recommended)', description: 'Validate key assumptions with something runnable as soon as possible.' },
        { label: 'Full design first', description: 'Converge on boundaries, protocols, and risks before implementing.' },
      ],
    },
    {
      id: 'signals',
      ...
      detail: 'Pick per the current hiring goal; skipping means no preference.',
      multiSelect: true,
      options: [
        { label: 'System design' },
        { label: 'Code quality' },
  ...
  const commandRemotes = {
    list(id: SessionId): RpcResult<readonly CommandDescriptor[]> {
      ...
      return {
        ...
        value: [
          ...
          { name: 'echo', description: 'fixture: echo the parameter', input: { hint: 'text to echo' } },
    ...
    execute(id: SessionId, line: string, images: readonly unknown[] = []): RpcResult<CommandExecution | undefined> {
      ...
      const outcomes: Record<string, string> = {
        compact: 'fixture: compacted (no-op)',
        echo: args.trim(),
        plan: args.trim() === 'off'
  ...
  const timingHooks = {
    ...
    beginModelRetry(id: string): void {
      ...
      append(sessionId, { type: 'assistant/chunk', data: { turn, step: 1, chunk: { type: 'text-delta', index: 0, text: 'The half reply that should be withdrawn' } } })
    },
    ...
    scheduleModelRetry(id: string, retry = 1, delayMs = 450): void {
      ...
      if (!scenario.stepStarted) {
        ...
        append(sessionId, { type: 'assistant/chunk', data: { turn: scenario.turn, step: 1, chunk: { type: 'text-delta', index: 0, text: `Reply ${String(retry)} that should be withdrawn` } } })
    ...
    completeModelRetry(id: string): void {
      ...
      append(sessionId, {
        ...
        data: {
          turn: scenario.turn,
          step: 1,
          message: assistantMessage(text('The full reply after the retry')),
        },
      })
      append(sessionId, { type: 'step/end', data: { turn: scenario.turn, step: 1 } })
  ...
  const api: ApiProxy = {
        ...
        startReply(
          ...
          userText === 'render markdown'
            ...
            : userText === 'report model'
              ...
              : `Echo: ${userText}. This is the fixture's streaming reply, for typewriter-growth and final-switch verification.`,
        )
        return ok(request, { accepted: true as const })
      },
      attachment: (request) => {
    ...
    skills: {
      list: (request) => {
        ...
        return ok(request, {
          skills: [
            { name: 'fixture-demo', description: 'fixture skill sample', whenToUse: 'UI directory rendering acceptance only', modelInvocable: true },
        ...
        if (approvalPending) {
          conn.push({
            ...
            payload: {
              ...
              toolName: 'dangerous_tool', reason: 'fixture persistent approval (answerable: approve/deny then it disappears)',
            },
          })
        }
        if (questionPending) {
