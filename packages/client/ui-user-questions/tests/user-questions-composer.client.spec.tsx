...
const QUESTIONS = [
  ...
  {
    id: 'signals', question: 'Pick the key signals (multi-select)', multiSelect: true,
...
describe('QuestionComposer', () => {
  it('collects single, custom, and multi-select answers before one batch submit', () => {
    ...
    expect(screen.getByText('Pick the key signals (multi-select)')).toBeTruthy()
    ...
    expect(respond).toHaveBeenCalledWith(answeredEnvelope('question-1', [
      { id: 'profile', selected: ['Shipping engineer (Recommended)'] },
      { id: 'detail', selected: [], custom: 'Must debug production issues independently' },
    ...
    const carrier = new PendingWait(
      ...
      SID,
      {
        questions: [{
          ...
          detail: '# Implementation plan\n\n- **Verify first** the status quo\n- Touch `QuestionComposer`',
  ...
  it('keeps IME Enter inside the custom input until composition finishes', () => {
    ...
    fireEvent.click(screen.getByRole('radio', { name: 'Research potential' }))
    const custom = screen.getByPlaceholderText('Type your answer')
    fireEvent.change(custom, { target: { value: 'typed text' } })
  ...
  it('surfaces cancellation failures: rejected receipt text and raw transport reasons', async () => {
    const respond = vi.fn()
      ...
      .mockRejectedValueOnce(new Error('second cancel failure'))
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Dismiss all questions' }))
    expect(await screen.findByText('second cancel failure')).toBeTruthy()
  })
  ...
  it('surfaces transport rejection and resets local drafts for a different request', async () => {
    ...
    expect(await screen.findByText('network dropped')).toBeTruthy()
...
describe('PendingQuestion domain face', () => {
  ...
  it('collapses the card to the header strip and expands it back', () => {
    ...
    expect(screen.getByText('Pick the candidate profile')).toBeTruthy()
  ...
  it('keeps the collapse toggle out of the cancel path and preserves drafts across collapse', () => {
    ...
    expect(respond).toHaveBeenCalledWith(answeredEnvelope('question-1', [
      { id: 'profile', selected: ['Shipping engineer (Recommended)'] },
      { id: 'detail', custom: 'Must debug production issues independently', selected: [] },
      { id: 'signals', selected: ['System design'] },
    ]))
  })
})
...
describe('parseRecommendedLabel', () => {
  it('recognizes English and Chinese suffixes without changing ordinary labels', () => {
    ...
    expect(parseRecommendedLabel('solid (recommended)')).toEqual({ label: 'solid', recommended: true })
    expect(parseRecommendedLabel('solid (recommended)')).toEqual({ label: 'solid', recommended: true })
