...
// The framework-injected t seat, stubbed over the shipped dictionary (the default locale).
...
const chip = () => screen.getByRole('button', { name: en['chip.on.aria'] })
...
describe('PlanChip', () => {
  ...
  it('executes /plan off once and follows the projection down', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByRole('button', { name: en['chip.on.aria'] })).toBeNull()
    })
  })
