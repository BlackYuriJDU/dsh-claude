...
/** Runtime with the locale face installed (the browser entry declares `locale:` — the shipped dictionary backs the t seat). */
...
describe('session rename through the assembled browser', () => {
  it('renames via the row menu: binding.session.rename fires, the dialog closes, the row re-labels from the list', async () => {
    ...
    fireEvent.change(input, { target: { value: '  Forked  experiment log  ' } })
    ...
    await waitFor(() => { expect(rename).toHaveBeenCalledWith('Forked  experiment log') })
    ...
    await runtime.sessions.updateSummary(SID, { displayTitle: 'Forked experiment log', title: 'Forked experiment log' })
    await view.findByText('Forked experiment log')
