...
describe('addHarnessSourceSection', () => {
  ...
  const EXPECTED = `The DSH implementation checkout is at ${SOURCE_ROOT}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend DSH itself.`
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      const identityAt = rendered.indexOf('You are an AI agent powered by DSH.')
