...
This fork's shipped posture is always full access: sessions default to the single `danger-full-access` permission preset (sandbox `danger-full-access` + approval `ask`), so the file sandbox never restricts mutations and the read-only / workspace-write ladder is not offered. Destructive commands (recursive deletes, force pushes, history discards, disk or power control, process sweeps, package removals, piped remote scripts) are gated instead: the shell tools ask the approval channel before running them, and a rejected or unavailable ask denies the command without executing anything. `DSH_PERMISSION_MODE` remains an escape hatch for the process sandbox fallback only. Stored General-settings permissions affect later Web sessions, not an already-open one.
