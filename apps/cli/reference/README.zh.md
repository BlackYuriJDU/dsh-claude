...
本 fork 的出厂姿态是始终完全访问：新会话默认使用唯一的 `danger-full-access` 权限预设（沙箱 `danger-full-access` + 审批 `ask`），文件沙箱不再限制任何修改，也不再提供 read-only / workspace-write 阶梯。取而代之的是破坏性命令闸门：shell 工具在执行递归删除、强推、丢弃历史、磁盘或电源控制、进程清扫、包卸载、管道远程脚本等命令前会先请求审批；被拒绝或无审批通道时命令不会执行。`DSH_PERMISSION_MODE` 仅保留为进程沙箱后备值的逃生舱。General settings 中存储的权限影响后续 Web 会话，不改变已打开的会话。
